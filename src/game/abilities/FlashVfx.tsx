...
import { FLASH_VFX_DURATION_MS } from "@/game/config/dodgeball.config";

const ORIGIN_PARTICLE_COUNT = 10;
const DEST_PARTICLE_COUNT = 14;
const HALO_BASE_SCALE = 1.6;

/** The fields FlashVfx needs each frame; supplied by any flash-like store. */
export interface FlashVfxSource {
...
/**
 * Pure flash burst renderer (halos + particle puffs). It owns no input or
 * gameplay — every frame it reads the latest cast from {@link read} and plays
 * the VFX. Used for both the local player (driven by useFlashStore) and the
 * networked opponent (driven by useOpponentFlashStore).
 */
export function FlashVfx({ read }: { read: () => FlashVfxSource }) {
  ...
  useFrame(() => {
    ...
    const elapsed = now - flash.lastCastAt;
    const active = flash.lastCastAt > 0 && elapsed <= FLASH_VFX_DURATION_MS;
    ...
    placeHalo(haloOriginRef.current, flash.lastOrigin, HALO_BASE_SCALE * (1 - t01 * 0.4), haloFade);
    placeHalo(haloDestRef.current, flash.lastDestination, HALO_BASE_SCALE * (1 + t01 * 0.5), haloFade * 1.1);

    // Particles puff outward from both points, drifting slightly.
    animateParticles(originParticles, flash.lastOrigin, t01, active, 0.85);
    animateParticles(destParticles, flash.lastDestination, t01, active, 1.0);
  });
